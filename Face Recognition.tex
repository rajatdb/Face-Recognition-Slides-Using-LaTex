...
\begin{document}
    ...
    \begin{itemize}
    \item The PCA and LDA algorithms are based on an efficient computation of Eigen values and Eigenvectors.
    \item PCA also known as \textbf{Karhunen Lower Transformation} is used to reduce the dimensionality.
    \item  In PCA the main aim is to reduce the data onto lower dimensional space also called as Eigen space by computing the Eigen values and Eigenvectors of dataset.
    \item The LDA computes the scatter matrix within class and scatter matrix between class thus separating the images within class increasing the recognition rate.
    \item The output of PCA is the input to LDA algorithm. It is based on Eigen value and Eigenvector.
    \end{itemize}
    \end{frame}     
